...
3. **Format before committing** — CI requires it: if the change includes `.md` files,
   run `bash .github/scripts/format.sh` (Prettier) and re-stage whatever it changes;
   if the project already has its own `[LINT_COMMAND]`/formatter, run it over the
   touched files too.
...
5. For AI-assisted work, add the attribution/co-author line in the footer as defined by `CONTRIBUTING.md` (a `Co-Authored-By:` trailer).
6. Create the commit. Show the final message to the user.
